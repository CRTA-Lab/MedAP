...
class ContourEditor:
    ...
    def ask_for_segmentation_mode(self):
        self.mode_popup = customtkinter.CTkToplevel(self.root)
        self.mode_popup.geometry("350x150")
        self.mode_popup.title("Choose Segmentation Mode")
        self.mode_popup.attributes("-topmost", True)
        self.mode_popup.grab_set()
        self.mode_popup.focus_force()

        label = customtkinter.CTkLabel(self.mode_popup, text="Select segmentation mode:", font=(self.font_size, self.font_size))
        label.pack(pady=12)

        self.segmentation_mode_var = customtkinter.StringVar(value="SAM")
        sam_button = customtkinter.CTkButton(
            self.mode_popup, text="Segment Anything (SAM)", font=(self.font_size, self.font_size),
            command=lambda: self.set_segmentation_mode("SAM")
        )
        custom_button = customtkinter.CTkButton(
            self.mode_popup, text="Custom Model", font=(self.font_size, self.font_size),
            command=lambda: self.set_segmentation_mode("Custom")
        )
        sam_button.pack(pady=8)
        custom_button.pack(pady=8)

        # Prevent closing without selection
        self.mode_popup.protocol("WM_DELETE_WINDOW", lambda: None)
        self.mode_popup.wait_window()
